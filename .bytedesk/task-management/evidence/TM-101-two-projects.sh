...
step "startup: launch cost does not scale with the number of agents"
# A deliberately slow agent: three seconds before it prints its ready line. The honest measurement
# is not an absolute time — a loaded machine moves that — but the SHAPE: a serial launch costs
# agents x ready-time, so going from one agent to three would roughly triple it. Flat means
# concurrent.
cat > "$ROOT/providers/slowfake.json" <<'JSON'
{
  ...
  "ready": { "pattern": "fake-agent ready", "tmux_pattern": "fake-agent ready", "delay_ms": 500, "timeout_ms": 30000 },
  "submit_keys": ["Enter"],
  "bootstrap_message": "Read {{bootstrap_file}} and follow it.",
  "detect": null,
  "install_hint": "test double; never installed",
...
# time_launch <name> <n-agents> -> sets LAST_MS and LAST_READY, and tears the session down.
# It sets globals rather than echoing, because command substitution would run it in a subshell and
# the counts would never come back. Declared up front for `set -u`.
LAST_MS=0
LAST_READY=0
time_launch() {
  local name=$1 n=$2 spec="$ROOT/$1.json"
  node -e '
    const n = Number(process.argv[1]), name = process.argv[2];
    const agents = Array.from({length: n}, (_, i) => ({
      id: `a${i}`, role: i === 0 ? "orchestrator" : "worker", cli: "slowfake",
    }));
    process.stdout.write(JSON.stringify({
      version: 1, name, agents,
      workflow: [{ stage: "go", from: "a0", to: ["a0"] }],
    }));' "$n" "$name" > "$spec"
  local s e out sess
  s=$(date +%s%N)
  out=$("$AO" launch --spec "$spec" --consumer "$P1" --providers-dir "$ROOT/providers" --json 2>/dev/null)
  e=$(date +%s%N)
  LAST_MS=$(( (e - s) / 1000000 ))
  LAST_READY=$(printf '%s' "$out" | node -e '
    let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{
      try {
        const r=JSON.parse(s.slice(s.indexOf("{")));
...
time_launch conc-one 1;   one_ms=$LAST_MS;   one_ready=$LAST_READY
time_launch conc-three 3; three_ms=$LAST_MS; three_ready=$LAST_READY
time_launch conc-ten 10;  ten_ms=$LAST_MS;   ten_ready=$LAST_READY


[ "${three_ready:-0}" = "3" ] \
  && ok "all three slow agents came up ready" \
  || bad "all three slow agents came up ready" "ready=$three_ready of 3"
[ "${one_ready:-0}" = "1" ] \
  && ok "the single-agent baseline came up ready" \
  || bad "the single-agent baseline came up ready" "ready=$one_ready of 1"

# The budget has to be derived from the ready time, not from a multiple of the baseline, or the
# test is vacuous. Launch carries a fixed overhead of roughly 3s on top of the agent's own 3s
# start, so the baseline is ~6s and a SERIAL three-agent launch would be ~12s — under a naive
# "2x the baseline" budget of ~12.4s, which would pass. Instead: a concurrent launch should cost
# about the baseline, so allow one extra ready-time of slack. Serial costs two extra and fails.
SLOW_MS=3000
budget=$(( one_ms + SLOW_MS ))
[ "$three_ms" -lt "$budget" ] \
  && ok "three agents cost ${three_ms}ms against a ${one_ms}ms baseline — flat, not per-agent" \
  || bad "three agents cost ${three_ms}ms against a ${one_ms}ms baseline" "a serial launch costs two extra ready-times; budget was ${budget}ms"

# Ten. Two distinct failures live here and neither shows up at three agents.
#
#  * Cost. Serial would be ~30 ready-times; a per-agent poll loop would grow the tmux client count
#    with the team. Push-based readiness means ten cost about what one does.
#  * Geometry. Ten panes split from one window used to fail outright with "no space for new pane" at
#    the seventh, and before that, narrow panes wrapped "fake-agent ready" across two rendered lines
#    where the server-side per-line search could never match it. Ten agents ALL reaching ready is
#    the assertion that catches both.
[ "${ten_ready:-0}" = "10" ] \
  && ok "all ten agents came up ready" \
  || bad "all ten agents came up ready" "ready=$ten_ready of 10"
ten_budget=$(( one_ms + (SLOW_MS * 2) ))
[ "$ten_ms" -lt "$ten_budget" ] \
  && ok "ten agents cost ${ten_ms}ms against a ${one_ms}ms baseline — ten cost what three do" \
  || bad "ten agents cost ${ten_ms}ms against a ${one_ms}ms baseline" "budget was ${ten_budget}ms; a serial launch would cost nine extra ready-times"
...
step "addressing: a spawn is named for who is running, and two spawns stay apart"
# A run of one library agent IS a spawn of that agent, so `tmux ls` should answer "who is running",
# not only "what ran". The discriminator's uniqueness scope is live sessions on this host, which is
# the scope tmux itself enforces — so it is probed, not assumed from entropy.
cat > "$ROOT/solo.json" <<JSON
{ "version": 1, "name": "some-task",
  "agents": [ { "id": "worker", "agent": "$P1_IMPL", "role": "orchestrator", "cli": "fake" } ],
  "workflow": [ { "stage": "go", "from": "worker", "to": ["worker"] } ] }
JSON
one=$("$AO" launch --spec "$ROOT/solo.json" --consumer "$P1" --providers-dir "$ROOT/providers" --json 2>/dev/null)
two=$("$AO" launch --spec "$ROOT/solo.json" --consumer "$P1" --providers-dir "$ROOT/providers" --json 2>/dev/null)
S1=$(printf '%s' "$one" | json .session); S2=$(printf '%s' "$two" | json .session)
D1=$(printf '%s' "$one" | json .runDir); D2=$(printf '%s' "$two" | json .runDir)
case "$S1" in
  "$P1_IMPL"-*) ok "the session is named from the agent's static id" ;;
  *) bad "the session is named from the agent's static id" "session=$S1 agent=$P1_IMPL" ;;
esac
printf '%s' "${S1#"$P1_IMPL"-}" | grep -qE '^[0-9a-f]{7}$' \
  && ok "and carries a per-spawn discriminator" \
  || bad "and carries a per-spawn discriminator" "suffix=${S1#"$P1_IMPL"-}"
[ -n "$S1" ] && [ -n "$S2" ] && [ "$S1" != "$S2" ] \
  && ok "two concurrent spawns of one agent get two distinct names" \
  || bad "two concurrent spawns of one agent get two distinct names" "$S1 vs $S2"
tmux has-session -t "=$S1" 2>/dev/null && tmux has-session -t "=$S2" 2>/dev/null \
  && ok "and both are separately addressable in tmux" \
  || bad "and both are separately addressable in tmux" "one of $S1 / $S2 is not live"

# Resolving in both directions: a name or title reaches the AGENT, a session reaches ONE SPAWN.
"$AO" agent show "$P1_IMPL" --consumer "$P1" --json 2>/dev/null | grep -q "$P1_IMPL" \
  && ok "resolving an agent by id reaches the agent" \
  || bad "resolving an agent by id reaches the agent" "agent show $P1_IMPL"
IMPL_NAME=$("$AO" agent show "$P1_IMPL" --consumer "$P1" --json 2>/dev/null | json .full_name)
[ -n "$IMPL_NAME" ] && [ "$("$AO" agent show "$IMPL_NAME" --consumer "$P1" --json 2>/dev/null | json .id)" = "$P1_IMPL" ] \
  && ok "resolving an agent by full name reaches the same agent" \
  || bad "resolving an agent by full name reaches the same agent" "name=$IMPL_NAME"
listed=$("$AO" session list --consumer "$P1" --providers-dir "$ROOT/providers" --json 2>/dev/null)
spawn_count=$(printf '%s' "$listed" | node -e '
  let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{
    try {
      const r=JSON.parse(s.slice(s.indexOf("{")));
      const row=(r.sessions||[]).find(x=>x.id===process.argv[1]);
      process.stdout.write(String(row?(row.spawns||[]).length:0));
    } catch { process.stdout.write("0"); }
  });' "$P1_IMPL")
[ "${spawn_count:-0}" = "2" ] \
  && ok "resolving a session reaches one spawn — both are listed under their agent" \
  || bad "resolving a session reaches one spawn — both are listed under their agent" "spawns=$spawn_count of 2"

# A team has no single answer, so it stays addressed by what ran and when.
team=$("$AO" launch --spec "$ROOT/team.json" --consumer "$P1" --providers-dir "$ROOT/providers" --json 2>/dev/null)
TS=$(printf '%s' "$team" | json .session); TD=$(printf '%s' "$team" | json .runDir)
case "$TS" in
  p1-*) ok "a multi-agent run stays addressed by what ran, not by one agent" ;;
  *) bad "a multi-agent run stays addressed by what ran, not by one agent" "session=$TS" ;;
esac
for d in "$D1" "$D2" "$TD"; do [ -n "$d" ] && "$AO" stop --run "$d" >/dev/null 2>&1; done

step "role-session: a named workspace that outlives the process that opened it"
# Every `ao-topology` invocation below is a SEPARATE process. That is the whole point: a role-session
# is durable only if a process that did not create it can find the same session again, so nothing
# here may be proved from state held in memory.
P1_ROLE=$("$AO" agent new --consumer "$P1" --role architect --cli fake | json .id)
opened=$("$AO" session open "$P1_ROLE" --consumer "$P1" --providers-dir "$ROOT/providers" --json 2>/dev/null)
SESS=$(printf '%s' "$opened" | json .session)
case "$SESS" in
  *"$P1_ROLE") ok "the session is named from the agent's stable id, not from a run" ;;
  *) bad "the session is named from the agent's stable id, not from a run" "session=$SESS id=$P1_ROLE" ;;
esac
[ "$(printf '%s' "$opened" | json .created)" = "true" ] \
  && ok "opening an agent that has no session creates one" \
  || bad "opening an agent that has no session creates one" "$opened"

# The second process must find the first process's session rather than starting a rival.
PANE1=$(printf '%s' "$opened" | json .pane)
again=$("$AO" session open "$P1_ROLE" --consumer "$P1" --providers-dir "$ROOT/providers" --json 2>/dev/null)
[ "$(printf '%s' "$again" | json .reattached)" = "true" ] \
  && ok "a second process reattaches instead of creating a duplicate" \
  || bad "a second process reattaches instead of creating a duplicate" "$again"
[ "$(printf '%s' "$again" | json .pane)" = "$PANE1" ] \
  && ok "and it is the same pane, so nothing in flight was lost" \
  || bad "and it is the same pane, so nothing in flight was lost" "$PANE1 vs $(printf '%s' "$again" | json .pane)"
[ "$(tmux list-sessions -F '#{session_name}' 2>/dev/null | grep -c "^${SESS}$")" = "1" ] \
  && ok "tmux holds exactly one session for this agent" \
  || bad "tmux holds exactly one session for this agent" "$(tmux list-sessions -F '#{session_name}' 2>/dev/null | grep "$P1_ROLE")"

# The record is what a gateway tab restore reads when the tmux session is GONE — it rebuilds from
# the stored command, so that command must not point into a run directory that has been torn down.
REC="$P1/.bytedesk/agent-orchestration/agents/$P1_ROLE/session.json"
[ -f "$REC" ] && ok "the record lives beside the agent, not inside a run" \
  || bad "the record lives beside the agent, not inside a run" "no $REC"
RESTORE=$(json .command < "$REC")
case "$RESTORE" in
  *"/runs/"*) bad "the restore command survives its run" "points into a run dir: $RESTORE" ;;
  "") bad "the restore command survives its run" "no command in $REC" ;;
...
tmux has-session -t "=$SESS" 2>/dev/null \
  && ok "the record alone rebuilds the session, which is what gateway restore does" \
  || bad "the record alone rebuilds the session, which is what gateway restore does" "restoring with: $RESTORE"

closed=$("$AO" session close "$P1_ROLE" --consumer "$P1" --providers-dir "$ROOT/providers" --json 2>/dev/null)
[ "$(printf '%s' "$closed" | json .closed)" = "true" ] && ! tmux has-session -t "=$SESS" 2>/dev/null \
  && ok "close ends the session" \
  || bad "close ends the session" "$closed"
[ -f "$REC" ] \
  && ok "and the agent survives it — closing a session does not retire an agent" \
  || bad "and the agent survives it — closing a session does not retire an agent" "$REC was removed"
