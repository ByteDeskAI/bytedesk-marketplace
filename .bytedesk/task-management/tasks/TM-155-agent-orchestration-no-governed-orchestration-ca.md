...
evidence: [".bytedesk/task-management/evidence/TM-155-HANDOFF.md"]
commits: ["caba55b"]
blockedBy: []
blocks: []
actor: "main"
...
updated: "2026-09-10T03:42:52.258Z"
...
priority: "medium"
evidenceSources: {".bytedesk/task-management/evidence/TM-155-HANDOFF.md":{"source":"/home/ryan/Documents/GitHub/ByteDeskAI/bytedesk-marketplace/.bytedesk/worktrees/TM-155-firstrun/.bytedesk/task-management/evidence/TM-155-HANDOFF.md","sha256":"e42df3a77c117b3f0c203841f839036e1b03664ef74fc199ff142518e83b32ad","bytes":5593,"at":"2026-09-10T03:42:52.120Z"}}
blockedReason: "Code complete and gated; blocked on the integrator's merge only. Branch tm/TM-155-first-run off main@435bb03, head 093f23b. Three first-run conditions, all found by running the demo four times and all knowable before launch: doctor reports CLAUDE_FOLDER_UNTRUSTED with the one-line remedy and states the question is per REPOSITORY not per agent directory; socketPathProblem answers the sun_path limit before tmux reports it as 'File name too long', which reads like a filename problem and is not; and promptErrorDetail puts the failing key into a prompt refusal that previously said only 'Invalid lead prompt; refusing restart'. AC2 is met by design rather than code, deliberately: a plugin that could answer a trust dialog would defeat the check, so the one human action is stated as part of the design the way failover.consent is. NOTE ON THE FIX ITSELF: running doctor live caught it reproducing this task's own original mistake — it asked about the exact path when trust is INHERITED by subdirectories, so it called a worktree of a trusted repo untrusted. Now walks ancestors and names which one answered; two regression tests. Gates: topology 364/364, both frozen presence validators unmodified, roadmap OK, doctor live on this repo reports only SUPERVISOR_NEVER_TICKED with trust.trusted true. HONEST WEAKNESS: the control against main is an IMPORT failure, not an assertion failure, because the exports are new — weaker evidence than the trap tests on my other branches; the behavioural claims rest on the demo transcripts."
---
