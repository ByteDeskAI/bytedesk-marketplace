...
export function specSchemaSummary() {
  return {
    ...
    fields: {
      ...
      stages: "ordered stages: { stage, from, to[], contract?, timeout?, wait_for?[], loop_until?, max_rounds?, description? }",
      workflow: "DEPRECATED spelling of `stages`. Still read, because specs are committed data — but `agents[].workflow` names a nested workflow, and one word cannot mean both.",
      "agents[].workflow": "name of another workflow, which joins this run as a TEAM rather than a pane: the conductor addresses it by id exactly like an agent. Takes `inputs` for the child; never a cli, model or candidates. With `for_each` it fans out into one child per item, addressed individually as `<id>.<item>` and collectively as `<id>`.",
      "agents[].for_each": "array, or one comma-separated string, expanded into one child per item on an `agents[].workflow` entry. `{{item}}` and `{{item.<key>}}` reach each child through its `inputs`.",
