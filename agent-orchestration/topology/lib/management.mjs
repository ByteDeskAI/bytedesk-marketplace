...
export async function recordLanding(options) {
  ...
  return withLock(join(ctx.root, 'integration.lock'), async () => {
    ...
    record.collected = true;
    await writeJson(ctx.path, record);
    const authorization = { decision: 'integrate', actor: actor.trim(), authorized: true, revision, channel: 'recorded-landing', reason: reason.trim(), policy_auto_merge: policy.auto_merge === true, at: nowIso() };
    const merge = { revision, landed, checks: [], target_branch: policy.target_branch, authorization };
