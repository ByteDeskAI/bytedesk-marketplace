...
import { ensureRunsIgnored, render } from "../../topology/lib/util.mjs";
...
test("the conductor is told to begin, and only the conductor", async () => {
  // Twice on claude:opus in clean repositories, a conductor read its brief, replied READY, and
  // stopped — with three healthy agents, an empty mailbox and no error anywhere. It was complying:
  // the pane message asks for READY, and the licence to start is the last line of a 118-line
  // document. So the instruction is completed rather than argued with.
  const adapter = (await loadAdapters([providersDir])).get("claude");
  const vars = { bootstrap_file: "/run/agents/x/BOOTSTRAP.md" };
  const base = render(adapter.bootstrap_message, vars);

  assert.ok(BEGIN_CLAUSE.includes("do not stop after READY"), "the clause says the thing that went wrong");
  assert.ok((base + BEGIN_CLAUSE).startsWith(base), "it rides on the bootstrap message, not a second send");

  // A WORKER must not get it. An agent that invented work for itself rather than waiting for mail
  // would be a worse bug than the one this fixes.
  for (const role of ["worker", "designer", "judge", "reviewer", "researcher", "implementer"]) {
    const pointer = base + (role === "orchestrator" ? BEGIN_CLAUSE : "");
