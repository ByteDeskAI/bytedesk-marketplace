...
export async function createServer(options = {}) {
  ...
  register(server, service, "ao_capabilities", "Describe orchestration providers, intents, protocols, permissions, lifecycle, and repository isolation guarantees.", {}, capabilitiesData, function () { return this.capabilities(); });
  ...
  register(server, service, "ao_list", "List runs belonging only to the explicit consumer repository.", { consumerCwd }, z.array(runData), service.list);
  register(server, service, "ao_events", "Read durable run events after a sequence number.", { ...runFields, after: z.number().int().nonnegative().optional() }, z.array(eventData), service.events);
  register(server, service, "ao_wait", "Wait up to 55 seconds for a run state change or terminal result.", { ...runFields, timeoutMs: z.number().int().positive().max(55_000).optional(), pollIntervalMs: z.number().int().positive().max(2_000).optional() }, runData, service.wait);
  register(server, service, "ao_send", "Start a cancellable child run that continues the final read-only provider session with a scoped follow-up message.", { ...runFields, message: z.string().min(1), timeoutMs: z.number().int().positive().max(7_200_000).optional() }, followupData, service.send);
  ...
  register(server, service, "ao_decision_approve", "Approve or reject an architecture decision with an auditable rationale.", { ...runFields, approved: z.boolean(), rationale: z.string().min(1), approvedBy: z.string().min(1) }, approvedDecisionData, service.approveDecision);
