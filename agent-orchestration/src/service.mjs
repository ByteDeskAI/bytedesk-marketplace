...
export class OrchestrationService {
  ...
  async terminateRecordedProcessGroup(worker) {
    if (worker?.supervisorUnit) {
      ...
      const readState = async () => {
        const { stdout } = await runFile("/usr/bin/systemctl", ["--user", "show", worker.supervisorUnit, "--property=LoadState", "--property=ActiveState"], { timeoutMs: 5_000 });
      ...
      try { await runFile("/usr/bin/systemctl", ["--user", "stop", worker.supervisorUnit], { timeoutMs: 10_000 }); } catch { return false; }
  ...
  async waitForWorkerRegistration(runId, supervisorUnit, child, launchState, timeoutMs = 10_000) {
    ...
    while (Date.now() < deadline) {
      ...
      if (run.worker?.attachedAt) {
        ...
        const scope = await runFile("/usr/bin/systemctl", ["--user", "show", supervisorUnit, "--property=LoadState", "--property=ActiveState", "--property=ControlGroup"], { timeoutMs: 2_000 }).catch(() => null);
  ...
  async launchWorker(runId) {
    ...
    try {
      child = spawn("/usr/bin/systemd-run", args, {
        cwd: this.pluginRoot,
  ...
  async worker(runId) {
    ...
    if (TERMINAL_STATES.has(run.state)) return run;
    const { stdout: controlGroup } = await runFile("/usr/bin/systemctl", ["--user", "show", run.worker?.supervisorUnit, "--property=ControlGroup", "--value"], { timeoutMs: 5_000 });
  ...
  async doctor() {
    ...
    const supervisor = await runFile("/usr/bin/systemd-run", ["--user", "--wait", "--collect", "--quiet", `--unit=${supervisorUnit}`, "/usr/bin/true"], { timeoutMs: 10_000 })
