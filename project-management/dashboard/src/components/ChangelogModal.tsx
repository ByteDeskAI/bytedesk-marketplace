...
const SECTION_APPEARANCE: Record<string, 'success' | 'inprogress' | 'removed' | 'moved' | 'default'> = {
  Added:   'success',
  Changed: 'inprogress',
  Fixed:   'moved',
  Removed: 'removed',
    ...
    fetch('/api/changelog')
      ...
      .then(body => {
        ...
        if (body.entries.length > 0) {
          setExpanded(new Set([body.entries[0].version]));
        }
      })
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 0' }}>
            ...
            {error && (
              <p style={{ padding: '20px 20px', fontSize: 13, color: 'var(--ds-text-danger)' }}>
                {error}
              </p>
            )}
              ...
              return (
                ...
                >
                  ...
                  >
                    {/* Chevron */}
                    <span style={{
                      fontSize: 10, color: 'var(--ds-text-subtlest)',
                    ...
                    {isLatest && (
                      <Lozenge appearance="new" isBold>Latest</Lozenge>
                    )}
                            ...
                            <ul style={{
                              ...
                              {section.items.map((item, ii) => (
                                <li key={ii} style={{
                                  ...
                                  {/* Strip leading/trailing ** if bold section header */}
                                  {item.replace(/^\*\*(.+)\*\*$/, '$1')}
                                </li>
                              ))}
                            </ul>
