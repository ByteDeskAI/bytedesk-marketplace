...
# --- compose: generated from shared + per-tool local, and never loses the local ---
setup
export PATH="$HOME/bin:/usr/bin:/bin"; mkdir -p "$HOME/bin" "$HOME/.grok"
printf '#!/bin/sh\n' > "$HOME/bin/grok"; chmod +x "$HOME/bin/grok"
printf 'GROK ONLY LINE\n' > "$HOME/.grok/AGENTS.md"
cp "$HOME/.grok/AGENTS.md" "$HOME/.agents/grok.local.md"
conf wire >/dev/null 2>&1
body=$(cat "$HOME/.grok/AGENTS.md")
case "$body" in
  *"GROK ONLY LINE"*) ok "compose keeps the per-tool local content" ;;
  *) bad "compose dropped the tool-specific content" "$body" ;;
esac
case "$body" in
  *"rule one"*) ok "compose includes the shared source" ;;
  *) bad "compose omitted the shared source" ;;
esac
case "$body" in
  *"GENERATED by agentconf"*) ok "the generated file warns against editing it" ;;
  *) bad "no generated-file banner — edits would vanish unexplained" ;;
esac
printf '\nhand edit\n' >> "$HOME/.grok/AGENTS.md"
conf check >/dev/null 2>&1
