...
- Optionally an **implementation URL** (a dev server, a Tauri window's Vite URL) and the
  stylesheet it uses. When given, the goal widens: the surface must load **that stylesheet**
  — `<link>` the implementation's global sheet (which itself imports the tokens) rather than
  vendoring the authority's tokens alone. Two sheets that are merely similar leave a
  pixelDiff floor that no patch round can remove; one shared sheet makes zero reachable,
  and every remaining difference is then a markup or rule difference the app can adopt.
...
built. Decide "invented" against what the product actually is, not against the picture:
read the state's brief, the requirements, or the app's own source when you have it, and
ask when you have none. A panel that describes something other than the state's subject
(image metadata behind a project dialog, a language/licence block on a design tool) is
the model filling space; a control the state genuinely needs but the mockup omitted is a
gap to fill, not an invention to copy. A measurement written down once saves a round for every state that shares the
layout, which is why the spec is per layout, not per state.

Type sizes are the measurement most often wrong, and the most expensive when wrong: a
mono size 0.5px too large wraps a line the mockup keeps on one, and every row below it
shifts. So for every text block, record a **wrap check** alongside the size: the longest
line's character count and the column width it sits in. `chars × 0.6 × size ≤ width` for
mono (≈0.5 for sans) is the arithmetic; if it fails, the size is wrong, not the column.
Two minutes here is the cheapest round you will ever not run.
...
hotspot, the spec is wrong, not the prompt: re-measure that region, correct `SPEC.md`,
and take **one** more round as a disclosed exception — not two. If that round does not
close it, stop and report the region as unresolved with the numbers; a fifth round is
where "least work" has already been lost.
