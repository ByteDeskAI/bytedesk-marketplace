...
export const GROUPS = [
  ...
  { id: "agents", label: "Agent dispatch", help: "The worker pool: who picks up ready-for-agent tasks, and how many at once." },
];
...
export const CATALOG = [
  ...
  },
  {
    ...
    label: "Fields an explicit create must carry",
    help: 'Field list from lib/completeness.mjs: "body", "acceptance", "evidence", "actor". Binds CLI/MCP/HTTP creates only — harness mirrors are exempt.',
  },
  {
    key: "requireOnStart",
    group: "policy",
    type: "json",
    default: ["body", "acceptance"],
    label: "Fields a task needs before start",
    help: "Same field list. Binds tm start and tm dispatch on every surface.",
  },
  {
    key: "requireOnDone",
    group: "policy",
    type: "json",
    default: ["body", "acceptance", "evidence", "actor"],
    label: "Fields a task needs before done",
    help: '"acceptance" here means at least one criterion exists — on top of requireAcceptance ticking them — so a task with no criteria cannot close. "actor" accepts actor or assignee.',
  },
  {
    key: "wipLimit",
    group: "policy",
  ...
  },
  {
    ...
    min: 5,
    max: 3600,
    label: "Pool tick interval (seconds)",
  },
  {
    key: "dispatch.heartbeatSeconds",
    group: "agents",
    type: "integer",
    default: 60,
    min: 0,
    max: 3600,
    label: "Claim heartbeat interval (seconds)",
  ...
  },
  {
    ...
    min: 1,
    max: 10_080,
    label: "Agent liveness TTL (minutes)",
    help: "A registered agent with no live pid and no heartbeat inside this window reads as dead; the reaper parks its tasks.",
  },
  {
    key: "ntfy.enabled",
    group: "ntfy",
