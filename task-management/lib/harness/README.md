...
| GoF | Role here |
|-----|-----------|
| **Bridge** | Domain store ops (`apply.mjs`) stay stable while wire implementors vary (Claude, Grok, …). |
| **Adapter** | `claude.mjs` / `grok.mjs` convert foreign tool payloads into `MirrorIntent[]` only. |
...
4. Unit-test wire → intents → apply without spawning Claude/Grok.
