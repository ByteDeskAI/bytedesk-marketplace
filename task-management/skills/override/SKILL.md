...
1. **Ask what's actually being bypassed** and whether the cheap fix is faster:
   - No active epic → `.bytedesk/task-management/bin/tm epic use <id>` is usually one command.
   ...
   - Unmet acceptance criteria → if the criterion is genuinely satisfied, tick it
     (`.bytedesk/task-management/bin/tm accept <id> <n>`); overriding here hides an unverified close.
2. `.bytedesk/task-management/bin/tm override "<reason>"` — arms a one-shot token. The next gate passes and clears it.
3. Retry the action that was blocked.

## Escalation

- Persistently wrong gate → change the policy, not the token:
  `.bytedesk/task-management/bin/tm config requireEpic false` · `.bytedesk/task-management/bin/tm config wipLimit 5` · `.bytedesk/task-management/bin/tm config requireAcceptance false`
