...
4. Mint only the tickets you can phrase **now**. Template via `.bytedesk/task-management/bin/tm task new --template interview|research|prototype|unblock` (a template ships the body and a criterion the create gate requires), or `tm_task_create` with `body` + `acceptance` + `labels` in one call — a bare title is refused. Wire `blockedBy` in a **second pass** (`tm_task_field` with `dep`).
